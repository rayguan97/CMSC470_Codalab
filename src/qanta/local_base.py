...
def accuracy_fn(logits, labels):
    ...
    ## first entry, do not buzz; second entry, buzz
...
def show(data_loader, model, device, ques_texts, guesses_and_scores, answers, b_size, id_in=0):
    """
    evaluate the current model, get the accuracy for dev/test set
    Keyword arguments:
    data_loader: pytorch build-in data loader output
    ...
    for idx, batch in enumerate(data_loader):
        if idx == 0:
            ...
            print_format(model, question_feature_vec, question_len, ques_texts, guesses_and_scores, answers, labels, srt_index=1)
            ## first entry, do not buzz; second entry, buzz



            # print(batch)
            # print(batch['feature_vec'].shape)
        # print(F.log_softmax(out, dim=1))

    return


def print_format(model, vec, quest_len, ques_texts, guesses_and_scores, answers, labels, srt_index=0, length=1):
    # out = model(vec[srt_index: srt_index + length], quest_len[srt_index: srt_index + length])
    # print(out)
    print(type(vec))
    for i in range(srt_index, srt_index + length):
        out = model(vec[i:i+1], [quest_len[i:i+1]])

        _, top_i = out.topk(1)
        prediction = top_i.squeeze()

        for j in range(len(ques_texts[i])):
            print(ques_texts[i][j])
            print(guesses_and_scores[i][j])
            print('Labels:')
            print(labels[i][j])
            print('predictions:')
            print(prediction[j])
        ...
        print(prediction)
        print('\n')
        print([x[0] for x in answers[srt_index:srt_index + length]])
    # logits = out.contiguous()
    # logits = logits.view(-1, 2)   #2 is the number of labels
    # top_n, top_i = logits.topk(1)
    # print(top_i)
...
if __name__ == "__main__":
    ...
    parser.add_argument('--n_guesses', '-g', type=int, default=10, help='number of top guesses to consider for creating feature vector')
    ...
    parser.add_argument('--see_test_accuracy', '-t', action='store_true', default=False, help='flag indicating seeing test result FINALLY after tuning on dev')
    ...
    parser.add_argument('--show', '-s', action='store_true', default=False, help='to show or not')
    parser.add_argument('--save-model', '-m', type=str, default='')
    ...
    if args.guesser_saved_flag or args.show:
    ...
    if args.buzz_data_saved_flag or args.show:
        train_exs = np.load('train_exs_{}.npy'.format(args.n_guesses))
        dev_exs = np.load('dev_exs_{}.npy'.format(args.n_guesses))
        test_exs = np.load('test_exs_{}.npy'.format(args.n_guesses))
        dev_ques_texts = np.load('dev_ques_texts_{}.npy'.format(args.n_guesses))
        dev_guesses_and_scores = np.load('dev_guesses_and_scores_{}.npy'.format(args.n_guesses))
        dev_answers = np.load('dev_answers_{}.npy'.format(args.n_guesses))
    else:
        ...
        #print(len(test_exs))
        np.save('dev_ques_texts_{}.npy'.format(args.n_guesses), dev_ques_texts)
        np.save('dev_guesses_and_scores_{}.npy'.format(args.n_guesses), dev_guesses_and_scores)
        np.save('dev_answers_{}.npy'.format(args.n_guesses), dev_answers)
        np.save('train_exs_{}.npy'.format(args.n_guesses), train_exs)
        np.save('dev_exs_{}.npy'.format(args.n_guesses), dev_exs)
        np.save('test_exs_{}.npy'.format(args.n_guesses), test_exs)
    ...
    #     if idx==1:
    #         print(batch['feature_vec'])
    #         print(batch['len'])
    #         print(batch['labels'])
    #         print(batch['feature_vec'].shape)
    #         print(batch['len'].shape)
    #         print(batch['labels'].shape)


    model = RNNBuzzer(n_input=args.n_guesses)
    model.to(device)
    print(model)

    if not args.train:
        model = torch.load("buzzer_model_{}_".format(args.n_guesses) + args.save_model + ".pt")
    else:
        for epoch in range(args.num_epochs):
            print('start epoch %d' %(epoch+1))
            train_acc, dev_acc = train(args, model, train_loader, dev_loader, device)
        torch.save(model, "buzzer_model_{}_".format(args.n_guesses) + args.save_model + ".pt")

    if args.show:
        show(dev_loader, model, device, dev_ques_texts, dev_guesses_and_scores, dev_answers, args.batch_size)
